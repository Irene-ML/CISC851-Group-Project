...
def uncorr_nstep(agent, sigma_ih, sigma_ho):
    ...
    """
    delta_ih = np.multiply(p.random.normal(0, 1, (agent.n_feature, agent.n_hidden)), sigma_ih)
    delta_ho = np.multiply(p.random.normal(0, 1, (agent.n_hidden, agent.n_out)), sigma_ho)
    agent.w_ih = agent.w_ih + delta_ih
    agent.w_ho = agent.w_ho + delta_ho
