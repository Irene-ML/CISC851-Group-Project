...
class Agent:
    ...
    def __init__(self, n_feature, n_hidden, n_out):
        ...
        self.n_out = n_out
        self.w_ih = np.random.rand(n_feature, n_hidden)
        self.w_ho = np.random.rand(n_hidden, n_out)
    ...
    def prediction(self, input_features, activation = "sigmoid"):
        ...
        if activation == "sigmoid":
            return self.sigmoid(np.dot(self.sigmoid(np.dot(input_features, self.w_ih)), self.w_ho))
        
        elif activation == "relu":
            return self.sigmoid(np.dot(self.relu(np.dot(input_features, self.w_ih)), self.w_ho))
